...
def make_index():
	...
	for root, dirs, files in walk(root_dir,topdown=True):
		for file in files:
			...
			file_ext = splitext(file)[-1].lstrip('.').lower()
...
def search (keywords=''):
	...
	try:
		...
		if re.search(keywords,basename(file),re.I) ]
	except:
		res_file = [ file
		for file in index_data.keys()
		if keywords.lower() in basename(file).lower() ]
		res_file.sort(reverse=True)
...
def update_status():
	global status
	textfield.text_color = 'black' if res_file else 'red'
	if textfield.text:
...
def rename_file (sender):
	...
	new_name_list = [ '{}|||{}'.format(val,idx)
	for idx,val in enumerate(name_list) ]
	new_name_list.sort(reverse=False)
	
	refile_list = [ res_file[int(item.split('|||')[1])]
	for item in new_name_list
	if name_list.count(item.split('|||')[0])>1 ]
	
	re_list = [ i
	for i in Counter(name_list).values() if i > 1 ]
	status = '当前列表有{}组重名文件'.format(len(re_list))
...
class TableViewDataSource (object):
	...
	def tableview_cell_for_row(self, tableview, section, row):
		...
		cell.content_mode = ui.CONTENT_SCALE_ASPECT_FILL
...
if __name__ == '__main__':
	...
	tableview.row_height = 70
	tableview.frame = (0,42,w,h-42)
	tableview.autoresizing = 'wh'
